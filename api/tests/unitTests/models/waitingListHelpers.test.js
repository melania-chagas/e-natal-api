...
describe('Testes para as funções auxiliares da classe WaitingListModel', () => {
  ...
  describe(`findOrCreateUser: deve encontrar ou criar um novo usuário no banco
  de dados`, () => {
    test(`Se o usuário não estiver cadastrado no banco de dados, deve
    cadastrá-lo e retornar o seu 'id'`, async () => {
    ...
    test(`Se o usuário já estiver cadastrado no banco de dados, deve
    retornar o seu 'id'`, async () => {
  ...
  describe("findEbookIdByTitle", () => {
    ...
    test("Deve encontrar e retornar o 'ebookId' a partir do título do ebook",
    async () => {
  ...
  describe("insertIntoTableWatingList", () => {
    test(`Deve popular a tabela intermediária 'WaitingList' com userId e
    ebookId`, async () => {
      await insertIntoTableWatingList(userId, ebookIdList);

      expect(WaitingList.findOrCreate)
        .toHaveBeenCalledTimes(ebookIdList.length);
